...
class Item:
    def __init__(self):
        ...
        self.unit_price_vat_excl:float = float(str(random.randint(0, 1000)) + '.' + str(random.randint(0, 99)))
        self.discount_percent:int = random.randint(0, 50)
        self.discount_absolute:float = round((self.quantity) * (self.discount_percent/100), 2)
        ...
        self.vat_absolute:float = round((self.price_vat_excl) * (self.vat_percent/100), 2)
        self.other_taxes:float = 0
        self.total:float = round(self.price_vat_excl + self.vat_absolute + self.other_taxes, 2)