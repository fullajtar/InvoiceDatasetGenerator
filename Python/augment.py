...
def add_noise(img):
    ...
    noise = tf.random.normal(
        shape=tf.shape(img), mean=0, stddev=deviation, dtype=tf.float32
    )
    img += noise
...
def load_and_preprocess_image(image_filename, images_folder, annotations_folder, X, y):
    ...
    img = load_img(
        img_path, target_size=(ORIGINAL_HEIGHT, ORIGINAL_WIDTH), color_mode="grayscale"
    )
    ...
    if GENERATE_BOUNDING_BOX_ANNOTATIONS:
        for element_id, _ in y.items():
            annotation_path = os.path.join(
                annotations_folder, element_id, image_filename
            )
...
def augment_annotations(image_filename, X, y, datagen, seed_iteration):
    for element_id, _ in y.items():
        ...
        original_annotation = np.expand_dims(
            np.array(y[element_id][image_filename]), axis=0
        )
        ...
        annotation_save_path = os.path.join(
            AUGMENTED_ANNOTATIONS_DIRECTORY, element_id, f"{image_filename}"
        )
...
def augment_image_and_annotations(image_filename, X, y, datagen, seed_iteration):
    ...
    if GENERATE_BOUNDING_BOX_ANNOTATIONS:
        for element_id, _ in y.items():
            ...
            annotation_generator = datagen.flow(
                original_annotation, seed=seed_iteration
            )
            augmented_annotation = annotation_generator.next()[0]
            annotation_save_path = os.path.join(
                AUGMENTED_ANNOTATIONS_DIRECTORY, element_id, f"{image_filename}"
            )
            augmented_annotation = (augmented_annotation[:, :, 0] * 255).astype(
                np.uint8
            )
            imageio.imwrite(annotation_save_path, augmented_annotation)
...
def augment():
    ...
    for image_filename in tqdm(image_filenames, desc="Augmenting", colour="Green"):
        augment_image_and_annotations(
            image_filename, X, y, datagen, random.randint(0, 4294967295)
        )
