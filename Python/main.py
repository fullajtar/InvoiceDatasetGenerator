...
def init_webdriver():
    ...
    chrome_options.add_argument(
        "--disable-infobars"
    )  # Disable the infobar for Chrome being controlled by automated software
...
def generate_bounding_box_annotations(
    fields_dict, template, driver, invoice_index, annotations_path
):
    # insert faked fields to HTML code
    for element_id, include_field in fields_dict.items():
        ...
        if paragraph:
            ...
            annotation_absolute_path = os.path.join(
                os.path.sep, ROOT_DIR, "..", "HTML", "annotations", f"{element_id}.html"
            )
            export_html_as_jpg(
                f"{element_id}/{invoice_index}",
                ORIGINAL_ANNOTATIONS_DIRECTORY,
                driver,
                annotation_absolute_path,
            )
    return None
...
def main():
    ...
    for template in INVOICE_TEMPLATES:
        ...
        for language in INVOICE_LANGUAGES:
            loaded_dict, delivery_methods, payment_methods = read_language_pack(
                language
            )
            soup_template_translated = translate_template(soup_template, loaded_dict)

            for i in tqdm(
                range(INVOICES_TO_GENERATE),
                desc=f"template: {template}; language: {language}",
                colour="Green",
            ):
                ...
                fields_dict, faked = generate_html_invoice(
                    delivery_methods, payment_methods, fields_dict, soup
                )
                export_html_as_jpg(
                    invoice_index, ORIGINAL_IMAGES_DIRECTORY, driver, html_output_path
                )
                if GENERATE_BOUNDING_BOX_ANNOTATIONS:
                    generate_bounding_box_annotations(
                        fields_dict,
                        "./HTML/output.html",
                        driver,
                        invoice_index,
    ...
    t = time.time() - t_start
    print(
        str(invoice_index - INVOICE_NAME_START_AT) + " invoices generated in -->  ",
        f"{t:.3f}",
        "seconds",
    )
    ...
    t = time.time() - t_start
    print(
        str(invoice_index - INVOICE_NAME_START_AT) + " invoices augmented in -->  ",
        f"{t:.3f}",
        "seconds",
        end="  ",
    )
